...
#' @param rePrm A logical value. If TRUE and when the IRT dichotomous model (e.g., 3PLM) or GRM is fit to data, 
...
#' @param rePrm.gpc A logical value. If TRUE and when (G)PCM is fit to data, the nominal model
...
#' @export
bring.flexmirt <- function(file, type=c("par", "sco"), rePrm = TRUE, rePrm.gpc = TRUE, n.factor=1) {
  ...
  if(type=="PAR") {
    results <- bring.flexPrm(file=file, rePrm=rePrm, rePrm.gpc=rePrm.gpc)
  }
...
#' @importFrom utils count.fields read.delim read.fwf read.table
bring.flexPrm <- function(file, rePrm = TRUE, rePrm.gpc = TRUE) {
